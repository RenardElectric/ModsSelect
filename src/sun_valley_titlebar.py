"""
Authors: PicoPlanetDev and RenardElectric
License: MIT
Source:  Sun-Valley-titlebar and ModsSelect
"""
...
class Menu:
    def __init__(self, menubar, header):
        self.menu = tk.Menu(menubar.menubar_frame, tearoff=False, selectcolor="#fafafa" if darkdetect.theme() == "Dark" else "#ffffff")
    ...
    def add_radiobutton(self, label, variable, command=None):
        if command is not None:
            self.menu.add_radiobutton(label=label, value=0, variable=variable, command=command)
        else:
            self.menu.add_radiobutton(label=label, value=0, variable=variable)

    def add_checkbutton(self, label, variable, command=None):
        if command is not None:
            self.menu.add_checkbutton(label=label, onvalue=1, offvalue=0, variable=variable, command=command)
        else:
            self.menu.add_checkbutton(label=label, onvalue=1, offvalue=0, variable=variable)

    def add_separator(self):
        self.menu.add_separator()

    def state(self, state):
        self.menubutton["state"] = state
