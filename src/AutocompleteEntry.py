"""
Authors: Mitja Martini, Russell Adams and RenardElectric
License: "Licensed same as original by Mitja Martini or public domain, whichever is less restrictive"
Source: https://mail.python.org/pipermail/tkinter-discuss/2012-January/003041.html and ModsSelect
...
class AutocompleteEntry(ttk.Entry):
    ...
    def handle_keyrelease(self, event):
        ...
        if len(event.keysym) == 1 or event.keysym in tk_umlauts:
            self.autocomplete()
        self.find_in_mods_tree()

    def find_in_mods_tree(self):
        mods_tree = gui_elements.mods_tree
        tree_items = mods_tree.get_tree_items()
        for iid in tree_items:
            if self.get() == mods_tree.item(iid, "text"):
                mods_tree.set_selection(iid)
                mods_tree.item(mods_tree.parent(iid), open=True)
                try:
                    if mods_tree.get_item_place(iid) > mods_tree.get_item_place(mods_tree.identify_row(mods_tree.winfo_height()-27)):
                        while mods_tree.get_item_place(iid) > mods_tree.get_item_place(mods_tree.identify_row(mods_tree.winfo_height()-27)):
                            mods_tree.yview_scroll(1, "units")
                    else:
                        while mods_tree.get_item_place(iid) <= mods_tree.get_item_place(mods_tree.identify_row(35)):
                            mods_tree.yview_scroll(-1, "units")
                except TypeError:
                    pass
